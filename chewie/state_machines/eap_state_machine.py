...
class FullEAPStateMachine(AbstractStateMachine):
    ...
    def strip_eap_from_radius_packet(self, radius):
        ...
        self.logger.debug("radius EAP: %s", eap_msg)
    ...
    def event(self, event):
        ...
        self.logger.debug("full state machine received event: %s", event)
        # 'Lower Layer' shim
        ...
        self.logger.debug('end state: %s', self.state)
        ...
        if self.eap_req:
            if (hasattr(self.eap_req_data, 'code') and self.eap_req_data.code == Eap.REQUEST) \
                ...
                self.logger.debug("outputting eap, '%s', src: '%s' port_id: '%s'",
        ...
        if self.aaa_eap_resp and self.aaa_eap_resp_data:
            if self.aaa_eap_resp_data.code == Eap.RESPONSE:
                self.logger.debug('outputing radius')
    ...
    def handle_logoff(self):
        ...
        self.logger.debug('client is logging off %s', self.src_mac)
    ...
    def handle_success(self):
        ...
        self.logger.debug('Yay authentication successful %s %s',
    ...
    def timer_expired_event_received(self, event):
        ...
        self.logger.debug("Expired Timer Event Received")
    ...
    def message_event_received(self, event):
        ...
        self.logger.debug('type: %s, message %s ', type(message), message)
        if event.port_id:
